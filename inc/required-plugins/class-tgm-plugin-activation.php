...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
         ...
         * @var string
         */
        public $menu = 'adler-install-plugins';
         ...
         * does the adler_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
        ...
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'Install Required Plugins', 'adler' ),
                'menu_title'                     => __( 'Install Plugins', 'adler' ),
                'installing'                     => __( 'Installing Plugin: %s', 'adler' ),
                'oops'                           => __( 'Something went wrong.', 'adler' ),
                ...
                'return'                         => __( 'Return to Required Plugins Installer', 'adler' ),
                'dashboard'                      => __( 'Return to the dashboard', 'adler' ),
                'plugin_activated'               => __( 'Plugin activated successfully.', 'adler' ),
                'activated_successfully'         => __( 'The following plugin was activated successfully:', 'adler' ),
                'complete'                       => __( 'All plugins installed and activated successfully. %1$s', 'adler' ),
                'dismiss'                        => __( 'Dismiss this notice', 'adler' ),
            );
            ...
            do_action_ref_array( 'adler_init', array( $this ) );
         ...
         *
         * We get the links in the admin notices to point to the adler page, rather
         ...
         * @return null Returns early if not the adler page.
         */
        ...
        public function thickbox() {
            ...
            if ( ! get_user_meta( get_current_user_id(), 'adler_dismissed_notice', true ) ) {
                add_thickbox();
            }
        ...
        public function install_plugins_page() {
            ...
            $plugin_table = new adler_List_Table;

            // Return early if processing a plugin installation action.
            if ( isset( $_POST['action'] ) && 'adler-bulk-install' == $_POST['action'] && $plugin_table->process_bulk_actions() || $this->do_plugin_install() ) {
                return;
            }

            ?>
            <div class="adler wrap">
                ...
                <form id="adler-plugins" action="" method="post">
                    <input type="hidden" name="adler-page" value="<?php echo $this->menu; ?>" />
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['adler-install'] ) && 'install-plugin' == $_GET['adler-install'] ) ) {
                check_admin_referer( 'adler-install' );
                ...
                $fields = array( 'adler-install' ); // Extra fields to pass to WP_Filesystem.
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'adler' ) . '">' . __( 'Return to the Dashboard', 'adler' ) . '</a>' ) . '</p>';
            ...
            elseif ( isset( $_GET['plugin'] ) && ( isset( $_GET['adler-activate'] ) && 'activate-plugin' == $_GET['adler-activate'] ) ) {
                check_admin_referer( 'adler-activate', 'adler-activate-nonce' );
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'adler' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'adler' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'adler' ) . '</a>'  : '';

                // Define all of the action links.
                $action_links = apply_filters(
                    'adler_notice_action_links',
                    array(
                        'install'  => ( current_user_can( 'install_plugins' ) )  ? $show_install_link  : '',
                        'activate' => ( current_user_can( 'activate_plugins' ) ) ? $show_activate_link : '',
                        'dismiss'  => $this->dismissable ? '<a class="dismiss-notice" href="' . add_query_arg( 'adler-dismiss', 'dismiss_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'adler', 'adler', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'adler', 'adler', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'adler' );
            }
        ...
        public function dismiss() {
            ...
            if ( isset( $_GET['adler-dismiss'] ) ) {
                update_user_meta( get_current_user_id(), 'adler_dismissed_notice', 1 );
            }
        ...
        public function actions( $install_actions ) {
            ...
            // Remove action links on the adler install page.
            if ( $this->is_adler_page() ) {
                return false;
            }
        ...
        /**
         * Determine if we're on the adler Install page.
         *
         * @since 2.1.0
         *
         * @return boolean True when on the adler page, false otherwise.
         */
        protected function is_adler_page() {
        ...
        public function update_dismiss() {
            ...
            delete_user_meta( get_current_user_id(), 'adler_dismissed_notice' );
...
if ( ! class_exists( 'adler_List_Table' ) ) {
    /**
     ...
     */
    class adler_List_Table extends WP_List_Table {

        /**
         * References parent constructor and sets defaults for class.
         *
         * The constructor also grabs a copy of $instance from the adler class
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'adler' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'adler' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'adler' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'adler' ) : __( 'Recommended', 'adler' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'adler' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'adler' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'adler' ) . ' %2$s">' . __( 'Install', 'adler' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'adler' ) . ' %2$s">' . __( 'Activate', 'adler' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
                                'plugin_name'          => $item['sanitized_plugin'],
                                'plugin_source'        => $item['url'],
                                'adler-activate'       => 'activate-plugin',
                                'adler-activate-nonce' => wp_create_nonce( 'adler-activate' ),
                            ),
                            network_admin_url( 'themes.php' )
                        ),
                        $item['sanitized_plugin']
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'adler' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'adler' ),
                'source' => __( 'Source', 'adler' ),
                'type'   => __( 'Type', 'adler' ),
                'status' => __( 'Status', 'adler' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'adler-bulk-install'  => __( 'Install', 'adler' ),
                'adler-bulk-activate' => __( 'Activate', 'adler' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'adler-bulk-install' === $this->current_action() ) {
                ...
                // Loop through each plugin to install and try to grab information from WordPress API, if not create 'adler-empty' scalar.
                $i = 0;
                foreach ( $plugin_installs as $plugin ) {
                    $api[$i] = plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) ? plugins_api( 'plugin_information', array( 'slug' => $plugin, 'fields' => array( 'sections' => false ) ) ) : (object) $api[$i] = 'adler-empty';
                    $i++;
                }
            ...
            if ( 'adler-bulk-activate' === $this->current_action() ) {
                check_admin_referer( 'bulk-' . $this->_args['plural'] );
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'adler' ), $imploded );
                }
...
if ( ! class_exists( 'WP_Upgrader' ) && ( isset( $_GET['page'] ) && TGM_Plugin_Activation::$instance->menu === $_GET['page'] ) ) {
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
             ...
             * set to true for the adler class.
             *
             * @since 2.2.0
             *
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'adler' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'adler' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'adler' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'adler' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'adler' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'adler' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'adler' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'adler' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'adler' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'adler' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'adler' ) . '</span><span class="hidden">' . __( 'Hide Details', 'adler' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'adler' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'adler' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'adler' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'adler' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'adler' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'adler' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'adler' ) . '</span><span class="hidden">' . __( 'Hide Details', 'adler' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'adler' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'adler' );
                }
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'adler' ) . '">' . __( 'Return to the Dashboard', 'adler' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
